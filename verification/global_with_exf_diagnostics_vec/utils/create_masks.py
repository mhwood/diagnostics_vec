...
for row in range(min_row,max_row+1):
    ...
    counter+=1
file_name = os.path.join('..','input','lateral_mask_east.bin')
...
for row in range(min_row,max_row+1):
    ...
    counter+=1
file_name = os.path.join('..','input','lateral_mask_west.bin')
mask.ravel(order='C').astype('>f4').tofile(file_name)
...
for col in range(min_col,max_col+1):
    ...
    counter+=1
file_name = os.path.join('..','input','lateral_mask_south.bin')
mask.ravel(order='C').astype('>f4').tofile(file_name)
...
for col in range(min_col,max_col+1):
    ...
    counter+=1
file_name = os.path.join('..','input','lateral_mask_north.bin')
mask.ravel(order='C').astype('>f4').tofile(file_name)
...
for col in range(min_col,max_col+1):
    for row in range(min_row, max_row + 1):
        ...
        counter+=1
file_name = os.path.join('..','input','surface_mask.bin')
mask.ravel(order='C').astype('>f4').tofile(file_name)
