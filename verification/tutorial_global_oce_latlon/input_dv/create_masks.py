...
for row in range(domain_shape[0]):
    ...
    counter+=1
file_name = os.path.join(os.getcwd(),'prime_meridian_mask.bin')
...
for col in range(domain_shape[1]):
    mask[row,col] = counter
    counter+=1
file_name = os.path.join(os.getcwd(),'equator_mask.bin')
mask.ravel(order='C').astype('>f4').tofile(file_name)
...
for row in range(min_row, max_row + 1):
    for col in range(min_col,max_col+1):
        mask[row,col] = counter
        counter+=1
file_name = os.path.join(os.getcwd(),'indian_ocean_mask.bin')
mask.ravel(order='C').astype('>f4').tofile(file_name)
